...
public class RoutingChipset_DynamicItemResponder extends RoutingChipset
{
    ...
    public SyncResponse getSyncResponse(ItemKey item, SyncResponse rival)
    {
        ...
        {
            InventoryWrapper inv = InventoryWrapper.wrapInventory(real).setSlotsFromSide(side).setFuzzy(fuzzyMode).setFuzzyPercent(fuzzyPercent[fuzzyDamageMode]);
            if (inv.hasItem(item))
            {
    ...
    {
        return EnumRoutingChip.DYNAMICITEMRESPONDER;
    }
}
