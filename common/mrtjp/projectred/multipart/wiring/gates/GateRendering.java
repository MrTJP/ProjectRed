...
/**
 * Gate renderings provides a bridge between a gate's bitmasked renderstate to the
 * renderer. Each time a rendering is checked, it gets a bitmask from the actual
 * gate, and sets all the states of the torches, pointers, etc. Then, the
 * GateDynamicRenderer renders things that are common in all gates, like torches and
 * pointers. Then, the gate rendering is asked if there is something it wants to
 * render specail just for that gate, like bundled cabling or levers. attached
 * to them.
 * 
 * @author MrTJP
 * 
