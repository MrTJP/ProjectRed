...
import mrtjp.projectred.exploration.IRetroGenerator;
...
public class RetroactiveWorldGenerator implements ITickHandler {
    ...
    private void runRetroactiveGeneration(World w1) {
        if (generationQueue.size() == 0) return;
        
        if (w1.getWorldTime() % 10 != 0) return;
        ...
        for (RetroGenQueue q : iterationQueue) {
            ...
            removeQueue.add(q);
            count++;
            if (count >= 32) break; // Dont do so many in one tick.
        }
