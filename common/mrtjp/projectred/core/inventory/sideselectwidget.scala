...
class WidgetSideSelect(x:Int, y:Int, width:Int, height:Int, scale:Double) extends GhostWidget(x, y, width, height)
{
    ...
    {
        if (button == 0 && ticksHeld < 5 && lastSideHovered >= 0)
        {
            onSideClicked(lastSideHovered)
            ticksHeld = 5
        }
    }
...
trait TWidgetSidePicker extends WidgetSideSelect
{
    ...
    abstract override def onSideClicked(s:Int)
    {
        toggleSide(s)
        super.onSideClicked(s)
    }
...
{
    private var color:Int = PRColors.LIME.rgb
    private var activeHighlight:Boolean = false
...
trait TWidgetSideTE extends WidgetSideSelect
{
    ...
    abstract override def draw() = TileEntityRenderer.instance.renderTileEntityAt(tile, -0.5, -0.5, -0.5, 0)
}
...
trait TWidgetSideStaticRotation extends WidgetSideSelect
{
    ...
    {
        val initial:Matrix4f = new Matrix4f
