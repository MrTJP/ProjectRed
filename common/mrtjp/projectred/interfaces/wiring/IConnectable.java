 ...
 * ways.
 * 
 * Note: Although wires implement IConnectable, do not use IConnectable to
 * determine if a wire is connected to you, as IConnectable only exposes
 * possible connections, and there are reasons for wires to not connect (e.g. if
 * there is a microblock in the way). Use
 * {@link IWire#wireConnectsInDirection(int, int)} to determine the actual
 * connections of a wire.
 */
public interface IConnectable {
	 ...
	 * 
	 * @param wire
	 *            The wire being checked.
	 * @param blockFace
	 *            The face the wire is on in the block containing it. -1 when
	 *            checking for jacketed wire connections.
	 * @param fromDirection
	 ...
	 * </pre>
	 * 
	 * In this situation, where A is the wire, # is a solid block and B is this
	 * block, X is right and Y is down, blockFace is -Y (up) and fromDirection
	 * is -X (left).
	 * 
	 ...
	 *            The wire being checked.
	 * @param blockFace
	 *            The face of *this block* the edge is adjacent to.
	 * @param fromDirection
