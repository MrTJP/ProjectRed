...
public class TileTurbineRotary extends TileMachineBase {
    ...
    public Container getContainer(EntityPlayer player) {
        ...
        ghost.addRestrictedSlot(0, _inv, 141, 47, new ISlotCheck() {
            ...
            public boolean isSlotAllowed(ItemStack stack) {
                if (rotation == 0) {
                    return stack != null && stack.getItem().itemID == ProjectRedExpansion.itemVAWT.itemID;
                } else {
                    return false;
                }
            }
    ...
    public void onBlockBreak() {
        BasicUtils.dropItem(worldObj, xCoord, yCoord, zCoord, new ItemStack(ProjectRedExpansion.blockMachines.blockID, 1, EnumMachine.TURBINEROTARY.meta));
    ...
    public boolean onBlockActivated(EntityPlayer player) {
        if (!player.isSneaking()) {
            player.openGui(ProjectRedExpansion.instance, ExpansionGuiHandler.rotaryID, player.worldObj, xCoord, yCoord, zCoord);
            return true;
        }
        return false;
    }
