...
//        updateRecipeIfNeeded()
    ...
    def writePlan():Unit = {
        ...
        if (craftHelper.recipe.isDefined && !isPlanRecipe) {
            val out = craftHelper.recipe.get.getCraftingResult(craftHelper.invCrafting)
            if (!out.isEmpty) {
                val stack = getStackInSlot(27)
                if (!stack.isEmpty)
                    PlanItem.savePlan(stack, (0 until 9).map(getStackInSlot).toArray, out)
            }
        }
    }
    ...
    def clearGrid(id: Int):Unit = {
        world.getEntityByID(id) match {
            case p:PlayerEntity => p.openContainer match {
...
class CraftingResultTestHelper
{
    ...
    val invCrafting = new CraftingInventory(new Container(null, -1) {
        override def canInteractWith(playerIn:PlayerEntity):Boolean = false
    }, 3, 3)
    ...
    def findRecipeFromInputs(w:World):Unit = {
        val r = w.getRecipeManager.getRecipe(IRecipeType.CRAFTING, invCrafting, w)
    ...
    def loadResultFromRecipe():Unit = {
        recipe match {
            case Some(r) =>
                val result = r.getCraftingResult(invCrafting)
...
class SlotProjectCrafting(player:PlayerEntity, tile: TileProjectBench, idx: Int, x: Int, y: Int)
    ...
    override def canTakeStack(player:PlayerEntity): Boolean = {
...
//        FMLCommonHandler.instance().firePlayerCraftingEvent(player, stack, tile.craftHelper.invCrafting)
        ...
        val nonnulllist = player.world.getRecipeManager.getRecipeNonNull(IRecipeType.CRAFTING, tile.craftHelper.invCrafting, player.world)
...
class ContainerProjectBench(playerInv:PlayerInventory, val tile:TileProjectBench, windowId:Int) extends NodeContainer(ExpansionContent.projectBenchContainer.get, windowId)
{
    ...
    def transferAllFromGrid() {
        for (i <- 0 until 9) if (getSlot(i).getHasStack)
            transferStackInSlot(playerInv.player, i)
    ...
    override def slotClick(id:Int, dragType:Int, clickType:ClickType, player:PlayerEntity):ItemStack = {
        ...
        super.slotClick(id, dragType, mode, player)
    }
        ...
        else if (9 until 27 contains from) //storage
        {
            if (stack.getItem.isInstanceOf[PlanItem]) {
                if (!getSlot(27).getStack.isEmpty && ItemKey.get(getSlot(27).getStack) != ItemKey.get(stack))
                    transferStackInSlot(playerInv.player, 27) //transfer existing stack
...
class GuiProjectBench(c:ContainerProjectBench, playerInv:PlayerInventory, title:ITextComponent) extends NodeGui(c, 176, 208, playerInv, title) {
    {
        ...
        clear.size = Size(8, 8)
        clear.clickDelegate = { () => c.tile.sendClearGridAction(Minecraft.getInstance().player.getEntityId) }
        addChild(clear)
    }
    ...
    override def drawBack_Impl(mouse: Point, rframe: Float) {
        ...
        val plan = c.tile.getStackInSlot(27)
        if (c.tile.isPlanRecipe && !plan.isEmpty) {
            val inputs = PlanItem.loadPlanInputs(plan)
            for (((x, y), i) <- GuiLib.createSlotGrid(48, 18, 3, 3, 0, 0).zipWithIndex) {
                val stack = inputs(i)
                if (!stack.isEmpty) {
                    fillGradient(x, y, x+16, y+16, EnumColour.GRAY.argb, EnumColour.GRAY.argb)
                    ItemDisplayNode.renderItem(this, Point(x, y), Size(16, 16), zPosition, false, stack)
                }
            }
        }

        getFontRenderer.drawString(title.getFormattedText, 8, 6, EnumColour.GRAY.argb)
        getFontRenderer.drawString(playerInv.getDisplayName.getFormattedText, 8, 116, EnumColour.GRAY.argb)
    }

    override def drawFront_Impl(mouse: Point, rframe: Float) {
        if (Minecraft.getInstance().gameSettings.keyBindSneak.isKeyDown || true)
