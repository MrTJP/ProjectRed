...
class ShapelessNBTCopyRecipe(id: ResourceLocation, group: String, output: ItemStack, ingredients: NonNullList[Ingredient]) extends ShapelessRecipe(id, group, output, ingredients) {
    ...
    def this(other: ShapelessRecipe) = this(other.getId, other.getGroup, other.getRecipeOutput, other.getIngredients)
...
class ShapelessNBTCopyRecipeSerializer extends ShapelessRecipe.Serializer {
    override def read(recipeId: ResourceLocation, buffer: PacketBuffer) = new ShapelessNBTCopyRecipe(super.read(recipeId, buffer))

    override def read(recipeId: ResourceLocation, json: JsonObject) = new ShapelessNBTCopyRecipe(super.read(recipeId, json))
}
