        ...
        else if (t instanceof TileMultipart)
        {
            ...
            for (int r = 0; r < 4; r++)
            {
                ...
                if (p instanceof IBundledEmitter)
                    signal = BundledCommons.raiseSignal(signal, ((IBundledEmitter)p).getBundledSignal(Rotation.rotationTo(pside, side^1)));
            }

            TMultiPart p = tmp.partMap(6);
            if (p instanceof IBundledEmitter)
                signal = BundledCommons.raiseSignal(signal, ((IBundledEmitter) p).getBundledSignal(side^1));
